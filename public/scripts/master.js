...
app.navBar = () => {
   ...
   const createItems = ['bio','projects','game?','contact'];
   const menuItems = [];
...
app.footer = (page) => {
   const footer = [{main:'footer',type:'hr'},{main:'footer',type:'div',cn:'footer-holder'},{main:'.footer-holder',type:'span',cn:'left-side',text:page},{main:'.footer-holder',type:'span',cn:'middle',html:'<span class="url">linkedin</span> || <span class="url">github</span>'},{main:'.footer-holder',type:'span',cn:'right-side',text:'Last Updated 11/21/2020'}];
   ...
   if(document.querySelectorAll('nav>ul li')){
      ...
      for(let i = 0; i < menu.length; i++){
         menu[i].addEventListener('click',(e) => {
            ...
            const func = typeof(app[e.target.innerText]) == 'function' ? e.target.innerText : e.target.innerText === 'game?' ? 'vs' : 'notSetup';
            app[func]();
         });
      }
   }
